...
export function effectiveStats(s) {
    let atk = s.stats.atk;
    let def = s.stats.def;
    let vt = s.stats.vt;
    let flowGainMult = 1;
...
export function spendVoucher(s, stat) {
    if (s.statVouchers <= 0)
        return false;
    const cur = s.stats[stat];
    const max = stat === "vt" ? CAPS.VT : stat === "atk" ? CAPS.ATK : CAPS.DEF;
