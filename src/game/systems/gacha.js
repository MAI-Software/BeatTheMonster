...
export const PULL_COST = { normal: 50, premium: 25 }; // premium uses premium currency
const NORMAL_ODDS = { common: 0.78, rare: 0.18, epic: 0.035, legendary: 0.005 };
const PREMIUM_ODDS = { common: 0.45, rare: 0.35, epic: 0.16, legendary: 0.04 };
...
function fragsForRarity(rarity) {
...
export function canPull(s, banner) {
    return banner === "normal" ? s.coins >= PULL_COST.normal : s.premium >= PULL_COST.premium;
}
export function pull(s, banner) {
    if (!canPull(s, banner))
        return null;
    if (banner === "normal")
        s.coins -= PULL_COST.normal;
    else
        s.premium -= PULL_COST.premium;
    ...
    if (!alreadyOwned && after >= threshold) {
        crafted = true;
        if (isFlow)
            s.ownedFlow.push(itemId);
        else
            s.ownedEquipment.push(itemId);
    }
    return { itemId, itemName, rarity, fragsGained: gained, crafted, isFlow };
}
export function fragInfo(s, id) {
