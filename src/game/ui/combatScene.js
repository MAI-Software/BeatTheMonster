...
export function runCombat(root, enemy, stats, flow, input, seed) {
    return new Promise((resolve) => {
      ...
      <div class="scene combat">
        ...
        if (input.videoEl && input.kind === "camera") {
            video.srcObject = input.videoEl.srcObject ?? null;
            video.style.display = "block";
        }
        else {
            video.style.display = "none";
        }
        const canvas = root.querySelector("#ring");
        const ctx = canvas.getContext("2d");
        const ehp = root.querySelector("#ehp");
        const php = root.querySelector("#php");
        const phptext = root.querySelector("#phptext");
        const comboEl = root.querySelector("#combo");
        const flowfill = root.querySelector("#flowfill");
        const flowlabel = root.querySelector("#flowlabel");
        ...
        let raf = 0;
        let quit = false;
        root.querySelector("#quit").onclick = () => {
            quit = true;
        };
