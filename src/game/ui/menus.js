...
export function renderTraining(app) {
    ...
    wireNav(app);
    app.root.querySelectorAll("[data-train]").forEach((b) => {
        b.onclick = () => { if (train(s, b.dataset.train)) {
            app.persist();
            renderTraining(app);
        }
        else
            app.toast("Sin monedas"); };
    });
    app.root.querySelectorAll("[data-vch]").forEach((b) => {
        b.onclick = () => { if (spendVoucher(s, b.dataset.vch)) {
            app.persist();
            renderTraining(app);
        } };
    });
}
export function renderEquip(app) {
    const s = app.save;
    ...
    wireNav(app);
    app.root.querySelectorAll("[data-flow]").forEach((b) => {
        if (!b.dataset.flow)
            return;
        b.onclick = () => { s.equippedFlow = b.dataset.flow; app.persist(); renderEquip(app); };
    });
    app.root.querySelectorAll("[data-gear]").forEach((b) => {
        if (!b.dataset.gear)
            return;
        b.onclick = () => {
...
export function renderGacha(app) {
    const s = app.save;
    ...
    wireNav(app);
    app.root.querySelectorAll("[data-pull]").forEach((b) => {
        b.onclick = () => {
            const res = pull(s, b.dataset.pull);
            if (!res) {
                app.toast("Sin monedas suficientes");
                return;
            }
            app.persist();
            const box = app.root.querySelector("#pull-result");
        ...
        <div>+${res.fragsGained} fragmentos ${res.crafted ? "· <b class='crafted'>¡DESBLOQUEADO!</b>" : ""}</div>
            ...
            b.disabled = !canPull(s, b.dataset.pull);
        };
    });
}
export function renderChallenges(app) {
    const s = app.save;
    const block = (title, list, scope) => `<h3>${title}</h3>` + list.map((ch) => {
    ...
    const achv = ACHIEVEMENTS.map((a) => {
        const ap = s.achievements.find((x) => x.id === a.id) ?? { tier: 0, progress: 0 };
    ...
    wireNav(app);
    app.root.querySelectorAll("[data-claim]").forEach((b) => {
        b.onclick = () => { if (claimChallenge(s, b.dataset.claim, b.dataset.scope)) {
            app.persist();
            renderChallenges(app);
        } };
    });
}
export function renderRanking(app) {
    const s = app.save;
...
function wireNav(app) {
    app.root.querySelectorAll("[data-nav]").forEach((b) => {
        b.onclick = () => app.go(b.dataset.nav);
    });
    const back = app.root.querySelector(".back");
    if (back)
