...
class Game implements App {
  ...
  startFight(enemyId: string, episodeId?: number) {
    ...
    const status = this.root.querySelector<HTMLDivElement>("#prefStatus")!;
    this.root.querySelector<HTMLButtonElement>("#cancelFight")!.onclick = () => this.go("campaign");
    const begin = async (useCamera: boolean) => {
      status.textContent = useCamera ? "Pidiendo cámara…" : "Cargando…";
      if (this.input) this.input.stop();
      this.input = await createInput(useCamera);
      if (useCamera && this.input.kind !== "camera") this.toast("Cámara no disponible — usando teclado");
    ...
    this.root.querySelector<HTMLButtonElement>("#useCam")!.onclick = () => begin(true);
    this.root.querySelector<HTMLButtonElement>("#useKb")!.onclick = () => begin(false);
  }
  ...
  private onFightEnd(enemyId: string, episodeId: number | undefined, r: any) {
    ...
    for (const ep of EPISODES) {
      for (const id of ep.enemies) {
        if (id === enemyId && frontierIdx === s.episodeProgress && r.won) s.episodeProgress++;
        frontierIdx++;
      }
    }
    this.persist();
